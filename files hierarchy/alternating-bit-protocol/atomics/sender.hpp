...
//Port definition
struct Sender_defs{
    struct packetSentOut : public web::out_port<int> { };
    struct ackReceivedOut : public web::out_port<int> {};
    struct dataOut : public web::out_port<Message_t> { };
    struct controlIn : public web::in_port<int> { };
    struct ackIn : public web::in_port<Message_t> { };
};
...
template<typename TIME> class Sender{
    public:
        ...
        cadmium::web::output::message_type get_state_message_type() {
        	vector<string> fields({ "packetNum", "totalPacketNum" });
        	string description = "packetNum is something, totalPacketNum is something else";

        	return cadmium::web::output::message_type("s_sender", fields, description);
        }
};     
